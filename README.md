...
**Note:** the main report provides the full output information for any kind of downstream processing. All other output types are there to simplify the analysis without R or Python / make it quicker. The numbers of precursors and proteins reported in different types of output files might appear different due to different filtering used to generate those, please see the descriptions above. All the 'matrices' can be reproduced from the main report using R or Python.  
